 ...
 *
 * This file was entirely generated by CLine VSCode extension using Mistral
 * Devstral-2512 model.
 */
...
esp_err_t oled_init(void) {
    ...
    oled_send_command(OLED_CMD_CHARGE_PUMP_EN);                // Use internal DC-DC
...
esp_err_t oled_update_display(void) {
    ...
    // The code above was fixed by Claude.ai:
    // In horizontal addressing mode, set full column and page ranges
    // then stream the entire framebuffer in one transfer
    oled_send_command(OLED_CMD_SET_COLUMN_ADDR); // Set column address
    oled_send_command(0x00); // Start: column 0
    oled_send_command(0x7F); // End:   column 127

    oled_send_command(OLED_CMD_SET_PAGE_ADDR); // Set page address
    oled_send_command(0x00); // Start: page 0
